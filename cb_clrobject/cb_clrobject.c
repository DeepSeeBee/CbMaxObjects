...
typedef struct _SCbClrObject
{
	...
	CObject_Out_Matrix_Info_Get_Func* mObjectOutMatrixInfoGetFuncPtr;
	CObject_Matrix_Out_Get_Func* mObjectMatrixOutGetFuncPtr;
...
void __stdcall Object_Out_List_Send(SCbClrObject* aSCbClrObjectPtr, void* aOutletPtr, long aOutletIdx)
{
		...
		)
	{
		...
		if (aAllocated)
		{
			...
			{
				Max_Log_Write(aSCbClrObjectPtr, "InternalError: MemoryLeak: How to deallocate atom-array?", 1); // TODO.
			}
		}
		if (aSymbolNamePtr)
		{
...
void __stdcall Object_Out_Matrix_Info_Get_Func_Set(SCbClrObject* aSCbClrObjectPtr, CObject_Out_Matrix_Info_Get_Func* aFuncPtr)
{
	if (aSCbClrObjectPtr)
	{
		aSCbClrObjectPtr->mObjectOutMatrixInfoGetFuncPtr = aFuncPtr;
	}
}

void __stdcall Object_Out_Matrix_Get_Func_Set(SCbClrObject* aSCbClrObjectPtr, CObject_Matrix_Out_Get_Func* aFuncPtr)
{
	if (aSCbClrObjectPtr)
	{
		aSCbClrObjectPtr->mObjectOutMatrixInfoGetFuncPtr = aFuncPtr;
...
void __stdcall Object_Out_Matrix_Send(SCbClrObject* aSCbClrObjectPtr, long aOutletIdx)
{
	if (aSCbClrObjectPtr)
	{
	}
...
void __stdcall Object_In_Matrix_Receive(SCbClrObject* aSCbClrObjectPtr, long aInletIdx, const TCHAR* aMatrixObjectName)
{
	if (aSCbClrObjectPtr)
	{
		...
		{
			//const TCHAR* aMethodName = "jit_matrix_getdata";
			//t_symbol* aClassNamePtr = jit_object_classname(aMatrixObjectPtr);
			//t_symbol* aMethodSymbolPtr1 = gensym("jit_matrix_getdata");
			//t_symbol* aMethodSymbolPtr2 = gensym("matrix_getdata");
			//t_symbol* aMethodSymbolPtr3 = gensym("getdata");
			//t_symbol* aMethodSymbolPtr = aMethodSymbolPtr1;
			//method aMethod = jit_object_getmethod(aMatrixObjectPtr, aMethodSymbolPtr);
			// This is, what i tried:
			//void* aResultPtr1 = jit_object_method_imp(aMatrixObjectPtr, aMethodSymbolPtr, &aDataPtr, 0, 0, 0, 0, 0, 0, 0);
			//void* aResultPtr2 = jit_object_method_imp(aMatrixObjectPtr, aMethodSymbolPtr, aMatrixObjectPtr, &aDataPtr, 0, 0, 0, 0, 0, 0);
			//void* aResultPtr1 = jit_object_method(aMatrixObjectPtr, aMethodSymbolPtr, &aDataPtr, 0, 0, 0, 0, 0, 0, 0);
			//void* aResultPtr2 = jit_object_method(aMatrixObjectPtr, aMethodSymbolPtr, aMatrixObjectPtr, &aDataPtr, 0, 0, 0, 0, 0, 0);
			//void* aResultPtr1 = jit_object_method(aMatrixObjectPtr, aMethod, &aDataPtr, 0, 0, 0, 0, 0, 0, 0);
			//void* aResultPtr2 = jit_object_method(aMatrixObjectPtr, aMethod, aMatrixObjectPtr, &aDataPtr, 0, 0, 0, 0, 0, 0);
			//void* aResultPtr1 = jit_object_method_imp(aMatrixObjectPtr, "jit_matrix_getdata", &aDataPtr, 0, 0, 0, 0, 0, 0, 0);
			//void* aResultPtr2 = jit_object_method_imp(aMatrixObjectPtr, "jit_matrix_getdata", aMatrixObjectPtr, &aDataPtr, 0, 0, 0, 0, 0, 0);
		...
		}
		{
			...
			//jit_object_method(aMatrixObjectPtr, "getinfo", aMatrixObjectPtr, &aMatrixInfo);
			//void* aDataPtr = 0;
			//jit_object_method(aMatrixObjectPtr, "getdata", aMatrixObjectPtr, &aDataPtr);
