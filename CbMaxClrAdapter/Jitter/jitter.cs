...
namespace CbMaxClrAdapter.Jitter
{
    ...
    public sealed class CMatrixData : CSealable
    {
        ...
        {
            this.ReallocateInternal(0, 0, new int[] { }, new int[] { }, 0);
        }
        ...
        public void Reallocate(int aSize, int aDimensionCount, int[] aDimensionSizes, int[] aStrides, int aPlaneCount)
        {
            this.CheckWrite();
            this.ReallocateInternal(aSize, aDimensionCount, aDimensionSizes, aStrides, aPlaneCount);
        }
        public void ReallocateInternal(int aSize, int aDimensionCount, int[] aDimensionSizes, int[] aStrides, int aPlaneCount)
        {
        ...
        public void SetImage(Bitmap aBitmap)
        {
            ...
            var aRowStride = aRowUsedBytes + (aRowUsedBytes % sizeof(int));
            var aDimensionStrides = new int[] { aPlaneCount, aRowStride };            
            var aPlaneSize = 1;
            var aByteCount = aDx * aDy * aPlaneCount * aPlaneSize;
            this.ReallocateInternal(aByteCount, aDimensionCount, aDimensionSizes, aDimensionStrides, aPlaneCount);
            var aBytePos = 0;
        ...
        CSingleTypeInlet<CMatrix>
    {
        ...
        internal override void Receive(CMessage aMessage)
        {
